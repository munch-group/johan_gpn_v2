...
class ModelforVEPModel(torch.nn.Module):
    ...
    def get_scores(self, input_ids_ref, input_ids_alt):
        # Apply no_grad to avoid memory usage for gradient computation
        with torch.no_grad():
    ...
    def forward(self, input_ids_ref_fwd=None, input_ids_alt_fwd=None, input_ids_ref_rev=None, input_ids_alt_rev=None):
...
def run_vep(
    ...
    print("Debug: Checking dataset before prediction")
    print(f"Dataset type: {type(variants)}")
    print(f"Dataset length: {len(variants) if hasattr(variants, '__len__') else 'Unknown'}")

    # Ensure the dataset is not empty
    if not variants or len(variants) == 0:
        raise ValueError("The dataset 'variants' is empty or invalid.")
...
if __name__ == "__main__":
    ...
    # Debugging: Load and inspect the variants file
    print("Debug: Loading variants file")
    variants_df = pd.read_parquet("/home/johanulstrup/johan_gpn/people/johanulsrup/johan_gpn/steps/analysis/gpn_arabidopsis/output/variants/all/variants_chr2.parquet")
    print(variants_df.head())
    print(f"Number of rows: {len(variants_df)}")
