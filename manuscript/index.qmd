...
```
The resulting plots, exemplified by Figure @fig-CPG_normalized_chr_ab for CpG islands on chromosome 8, confirmed our expectation: there was no meaningful difference between A→B and B→A transitions. This indicates that compartment boundaries behave symmetrically and supports the conclusion that enrichment patterns observed in the main analysis reflect genuine biological signal rather than artifacts of compartment orientation.

::: {#fig-CPG_normalized_chr_ab fig-cap= "Negative control analysis of CpG islands on chromosome 8 across five macaque cell types (fibroblast, spermatogonia, pachytene spermatocyte, round spermatid, and sperm). A→B and B→A compartment transitions were compared."}
