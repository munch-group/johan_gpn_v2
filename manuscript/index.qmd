...
## GPN arcitecture
The idea for this project was to evaluate the Genome-Pretrained Network (GPN) introduced in @Kantorovitz2024 \cite{Kantorovitz2024} and determine whether it could achieve greater accuracy than traditional methods in predicting genome-wide variant effects.
...
We based our implementation work on the official **GPN** repository maintained by Song Lab: [songlab-cal/gpn](https://github.com/songlab-cal/gpn).

In particular, we referred to the **`analysis/gpn_arabidopsis`** subdirectory of the repository as an example of end-to-end model application on a plant genome: [gpn_arabidopsis](https://github.com/songlab-cal/gpn/tree/main/analysis/gpn_arabidopsis).
...
## Fecting the raw data Hic analysis 
...
For chromosome 8, no significant differences were observed between the A→B and B→A splits across tissues, with the exception of spermatogonia, where a deviation appeared in the region spanning 0.25 Mb to 1.25 Mb.  

For chromosome X, the results varied by tissue. In fibroblasts, the B→A split showed consistently higher GC content compared to A→B within the 0–1 Mb region. A similar pattern was observed in sperm, where GC content remained higher in B→A. By contrast, in round spermatids and pachytene spermatocytes, there were no significant differences between the two splits. In spermatogonia, however, GC content was again higher in B→A compared to A→B.  


::: {#fig-GC_content_contol }
![](illustrations/gc_compartment_analysis_chrX_chr8_combined_control_20250925_091502.png){width=100%}

 Control analysis of GC content for chromosomes 8 and X across five tissues. For chromosome 8, no significant differences between A→B and B→A were detected except in spermatogonia (0.25–1.25 Mb). For chromosome X, higher GC content was observed in B→A compared to A→B in fibroblasts (0–1 Mb), sperm (0–1 Mb), and spermatogonia, while round spermatids and pachytene spermatocytes showed no differences.
:::
...
Negative control analysis of CpG islands on chromosome 8 across five macaque cell types (fibroblast, spermatogonia, pachytene spermatocyte, round spermatid, and sperm). A→B (AB) and B→A (BA) compartment transitions were compared.
