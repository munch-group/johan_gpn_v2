...
keywords:
  ...
  - Genome Pretrained Networks (GPN)
abstract: |
 The three-dimensional organization of the genome plays a fundamental role in gene regulation, evolution, and germ cell development. Chromosomes are partitioned into transcriptionally active A-compartments and repressive B-compartments, with boundaries between them representing dynamic sites of chromatin reorganization. This thesis investigates the sequence determinants underlying these compartment boundaries in Macaca genomes, focusing on spermatogenesis and the distinctive regulatory behavior of the X chromosome.
 Initially, this study aimed to leverage Genome Pretrained Networks (GPNs) to predict chromatin states and evolutionary conservation from DNA sequence data. While conceptually promising, this approach proved technically challenging due to inadequate documentation of published workflows and significant hardware limitations. Even with multi-GPU configurations, full-genome training remained computationally infeasible, constraining analyses to chromosome-level embeddings. These obstacles underscore the practical difficulties of implementing cutting-edge deep learning methods in genomics research within typical academic resource constraints.
 The study therefore pivoted to classical bioinformatic approaches, examining sequence features at A/B compartment boundaries across five spermatogenic developmental stages. GC content and CpG islands emerged as strong predictors of compartment identity on autosomes, exemplified by chromosome 8, but these correlations were notably weakened or reversed on the X chromosome, reflecting its unique regulatory landscape. CpG islands demonstrated dynamic, stage-specific enrichment patterns in A-compartments during later spermatogenic stages, coinciding with periods of extensive chromatin remodeling. Repetitive elements exhibited contrasting distribution patterns: enriched in B-compartments on autosomes but preferentially located in A compartments on the X chromosome. Importantly, repetitive elements consistently clustered near compartment boundaries regardless of chromosomal context, suggesting potential structural or regulatory functions in chromatin organization.
 These findings demonstrate that while autosomes generally conform to established sequence-compartment relationships, the X chromosome exhibits distinct organizational principles that deviate from canonical patterns. This work establishes a framework for integrating sequence composition, repetitive elements, and chromatin architecture, while highlighting the critical importance of considering both biological context and methodological constraints when interpreting three-dimensional genome organization.
plain-language-summary: |
  In plain language, the thesis describes an analysis of something. Somethings was not previously know.
  I found something and was able to conclude something. The perspectives of this result is something.
key-points:
  - GC content and CpG islands reliably predict A/B compartment identity on autosomes, but not on the X chromosome.  
  - Chromatin transitions during spermatogenesis show stage-specific and chromosome-specific associations with CpG islands and repeats.  
  - Simple and nested repeats cluster at compartment boundaries, suggesting structural or modulatory roles in genome folding.  
  - Control analyses reveal both genuine biological asymmetries and possible methodological artifacts, emphasizing the need for independent validation.  
  - Attempts to apply Genome Pretrained Networks (GPNs) highlight both the promise of AI in genomics and the practical challenges of reproducibility under limited resources.  
date: last-modified
date-format: long
...
# Introduction
The central aim of this thesis is to investigate chromosomal regions where chromatin transitions between A- and B-compartments, specifically focusing on the sequence features and regulatory elements that accompany these boundaries. These transitions are of particular interest because they represent dynamic sites of genome reorganization, linking sequence composition to higher-order chromatin structure and, ultimately, to functional outcomes in gene regulation and evolution.

The original idea for this project was to leverage artificial intelligence—specifically genomic pre-trained networks (GPNs)—to predict conservation across species and compare these predictions with chromatin compartment organization. While technically ambitious, this initial approach proved infeasible within the scope of the project, primarily due to computational limitations and the complexity of reproducing large-scale models. Rather than abandoning the question, the project pivoted toward more classical and tractable methodologies, extending the investigation of chromatin transitions through established bioinformatic and comparative genomic approaches.

This transition in strategy opened up the possibility of exploring compartment boundaries from multiple complementary angles: analyzing sequence features such as GC content, CpG islands, and repetitive elements; assessing their distribution relative to compartment edges; and considering their potential roles in genome function and evolution. In doing so, the work contributes both methodological insights—on the challenges of applying AI in genomics—and biological insights into how compartmental transitions are structured and maintained.

## evolution on X
The production of gametes in sexually reproducing organisms is a complex, tightly regulated process involving numerous cellular and molecular mechanisms. In males mammals, spermatogenesis—the process by which sperm are formed—progresses through four main stages of differentiation: from germ cells to spermatogonia, then to pachytene spermatocytes, round spermatids, and finally to mature spermatozoa [@Wang2019]. This process is fundamental to male fertility. Meiosis, a specialized form of cell division, ensures the proper pairing, recombination, and segregation of homologous chromosomes, thereby maintaining genetic integrity and promoting variation. A deep understanding of these molecular events is essential for comprehending inheritance, speciation, population genetics, and the biological causes of infertility[@Wang2019].

Sex chromosomes differ from autosomes in several key ways due to their distinct patterns of inheritance and copy number. The Y chromosome is found only in males and exists as a single copy. In contrast, the X chromosome follows a more complex inheritance pattern: females carry two copies, while males carry only one. Consequently, the X chromosome resides two-thirds of the time in females and one-third in males, a distribution that influences how selection acts upon it. In males, the hemizygous nature of the X chromosome means that any mutations or deleterious alleles are fully exposed, without a second copy to mask their effects[@Abe2020].

This feature is central to Haldane’s rule [@Haldane1922], which posits that in hybrids between two species, the heterogametic sex (e.g., XY in mammals or ZW in birds) is more likely to be inviable or sterile. While widely observed, the precise causes of this pattern remain a subject of ongoing debate. Several hypotheses have been proposed to explain it. One suggests that incompatibilities involving the Y chromosome can arise if it fails to remain compatible with the X chromosome or autosomes[@Haldane1922]. Another centers on dosage compensation, proposing that hybridization may disrupt the regulatory mechanisms that balance gene expression between sex chromosomes. The dominance hypothesis argues that recessive deleterious alleles may be unmasked in the heterogametic sex, leading to sterility. Additional ideas include the faster evolution of male-biased reproductive genes, which can result in functional mismatches, and the accelerated divergence of X-linked loci compared to autosomes, known as faster-X evolution. Lastly, meiotic drive—genetic conflicts between selfish elements on sex chromosomes—has also been implicated in hybrid sterility.

These hypotheses underscore the complex interplay between sex chromosomes, selection, and hybridization [@Cowell2023]. Although Haldane’s rule is consistently observed across diverse taxa—and even across kingdoms—the underlying mechanisms are not universally agreed upon. In many cases, multiple processes may act together [@Lindholm2016].
...
this sudy aims characteriz these chromatin state transitions, with a particular interest in understanding how they are distributed across the genome and whether they exhibit predictable patterns linked to specific sequence features or regulatory elements. This study examines whether certain genomic regions are more prone to undergo structural reorganization and whether such regions harbor conserved sequence motifs, epigenetic signatures, or binding sites for key regulatory proteins [@zhou2011dna; @ernst2011mapping]. Furthermore, this study investigates whether particular chromatin configurations are disproportionately associated with genes involved in critical biological processes or with regions of the genome that show signatures of evolutionary constraint or adaptation [@kundaje2015integrative].

By examining the diversity of chromatin states and transitions across individuals and lineages, this study also aima to uncover broader principles governing chromatin architecture and its functional consequences [@marx2019singlecell]. These insights may inform our understanding of how chromatin organization evolves, how it contributes to phenotypic diversity, and how its dysregulation may underlie disease states [@buenrostro2015atacseq]. Ultimately, this work contributes to a growing body of knowledge on the interplay between genome structure, function, and evolution.
...
This studys leverages GPN-derived embeddings and predictions to investigate the sequence determinants of chromatin organization. This study aims to identify genomic regions where the GPN model predicts strong, conserved regulatory activity, and determine whether these regions also show structural stability in chromatin compartments across individuals or hybrid genomes. This approach allows us to assess whether chromatin transitions are driven primarily by changes in DNA sequence, or whether other regulatory layers (e.g., epigenetic modifications, chromatin remodelers) are required to explain shifts between A and B compartments.

By integrating Genomic Pre-trained Network (GPN) predictions with chromatin conformation data such as Hi-C, researchers can construct a powerful multiscale framework for understanding genome function—linking base-pair regulatory logic with higher-order nuclear architecture [@Benegas2023; @Schultz2025]. These models not only enhance the interpretation of noncoding variation and functional conservation but also offer a means to study how genomic sequence encodes three-dimensional chromatin organization. In systems such as primates, where population structure and hybridization events are prevalent, this approach opens new avenues for investigating the evolution of regulatory architecture in action [@Eres2020; @Alvarez2024].


## Meiosis 

Meiosis is the defining process of gametogenesis, transforming diploid precursors into haploid gametes while introducing genetic diversity. The stages represented in this dataset highlight distinct steps in this transformation[@Berg2019].
...
## Geneome-Pretrained Network (GPN) arcitecture
The idea for this project was to evaluate the GPN introduced in @Kantorovitz2024 and determine whether it could achieve greater accuracy than traditional methods in predicting genome-wide variant effects.

The model was designed as a convolutional neural network (CNN) and took input sequences with a window size of 512. During training, 15\% of the positions within each window were masked to enable the model to learn meaningful representations.

The architecture consisteds of 25 layers, each structured as follows: a dilated convolution layer, followed by an add-and-norm layer with a skip connection from before the dilated convolution. This was followed by a feedforward layer, another add-and-norm layer, and additional skip connections.

A feedforward layer was a fundamental component of neural networks, where inputs passed through one or more fully connected layers with activation functions, transforming data without looping back. This structure helps the model learn complex representations by applying weighted transformations and non-linearities @Kantorovitz2024.

A dilated convolution expandeds the receptive field of a convolutional layer without increasing the number of parameters or reducing resolution. By spacing kernel elements apart, it captured long-range dependencies in sequences, mada it particularly useful in genomic data analysis. When combined, dilated convolutions and feedforward layers enhanced a model’s ability to recognize patterns across different scales efficiently.

After passing through the 25 layers, the model produced a contextual embedding with a dimension of 512 (D=512), followed by classification layers. The final layer outputs the probabilities of the four nucleotides at each masked position.

The GPN variant effect prediction score was calculated as the log-likelihood ratio between the alternate (ALT) and reference (REF) alleles. Here, L represents the window length in base pairs, and D denotes the embedding dimension.


::: {#fig-fig_1_kantorovitz}
![](illustrations/fig_1_kantorovitz.png){width="50%"}

**Figure 1.** Schematic overview of the Genome-Pretrained Network (GPN) architecture, adapted from @Kantorovitz2024. The model processes genomic sequences using stacked dilated convolutional and feedforward layers with skip connections, producing contextual embeddings that support masked nucleotide prediction and variant effect scoring.
:::
...
We based our implementation work on the official **GPN** repository maintained by Song Lab: https://github.com/songlab-cal/gpn

In particular, we referred to the **`analysis/gpn_arabidopsis`** subdirectory of the repository as an example of end-to-end model application on a plant genome: https://github.com/songlab-cal/gpn/tree/main/analysis/gpn_arabidopsis.


## Data Preparation and Pretraining Workflow
To evaluate the GPN, the initial step involved reproducing the pretraining workflow provided in the original publication @Kantorovitz2024. This workflow outlines the procedure for generating a dataset compatible with the model, specifically for masked language modeling over genomic sequences.

A dataset suitable for GPN pretraining was successfully generated. The resulting data consisted of masked genomic sequences, formatted according to the model’s input requirements.

## Model Execution Workflow and Hardware Constraints

The step after the successful generation of a pretraining-compatible dataset involved attempting to run the GPN using the available inference and training workflows. The documentation provided on the model's GitHub repository lacked detailed guidance.

To proceed, an alternative workflow was identified, analysis/gpn_arabidopsis specifically designed for use with the Anatoptis genome. This workflow was adopted as a reference implementation for executing the GPN model.
...
As a result, attempts to execute the GPN model encountered frequent out-of-memory errors, particularly during forward passes through the deeper layers of the network. These memory constraints posed a major obstacle and significantly hindered reproducibility under the available hardware setup.

Initial efforts to train or fine-tune the GPN focused on applying the workflow to a baboon genome using a multi-GPU setup. The goal was to replicate the original training environment, which employed four NVIDIA A100 GPUs (80 GB RAM each). However, the available infrastructure utilized NVIDIA L43R GPUs, each with 45 GB of RAM, which introduced several compatibility and performance challenges.
...
## Fecting the raw data Hi-c analysis 
The original sequencing data was obtained by a colleague in our research group in order to reproduce the results reported by [@Wang2019]. The raw data was downloaded from the SRA portal [@Sayers2022] and subsequently filtered to include only paired-end Hi-C reads from macaque samples.

To provide an overview of the material, a summary of the SRA runtable is presented in @tbl-cell-data. The accession numbers alongside key metadata for each sample are listed in the table. Altogether, the dataset contained 1 TB of compressed FASTQ files, comprised ~9.5 billion reads distributed roughly evenly across five tissue types.
...
The Hi-C sequencing data analyzed in this study was not generated or processed by the author, but by an another member of our research group. To enable clarity and reproducibility, the following section contains a condensed description of their workflow.
...
## Datahandeling before the analysis
To load the data the function parse_compartment_data(file_name) was used, which transforms the raw eigenvector track into meaningful genomic segments. The function began by reading a CSV file that contains at least one column, e1, representing eigenvector values at 100 kb resolution.

The eigenvector track (E1) marked A/B compartments along the genome. During its generation, some bins may contained NaN values. If these were left in the dataset, each missing value was treated as a break and produces false edges, creating compartment transitions that did not actually exist. To avoid this problem, all rows containing NaN values was removed before proceeding: 
``` python
     e1_100kb.dropna(inplace=True)
```
This step ensurd the sign changes in the eigenvector reflect only genuine compartment transitions.

After cleaning, the function calculated genomic coordinates by assigning each bin a start and end position based on its index and the fixed bin size of 100 kb. With these coordinates in place, it determined the sign of each eigenvector value and assigns segment IDs whenever the sign changes, so that continuous stretches of positive or negative values are grouped into compartments.

Each compartment was then summarized by calculating the mean and sum of its eigenvector values and recording the minimum start and maximum end positions. Segments with a positive mean were classified as compartment A, while those with a negative mean are classified as compartment B. To prevent artifacts caused by missing bins, the function also checks segment borders to ensure that gaps do not create artificial extents.

The function produced two outputs. The first was a tidy data frame of compartments that includes chromosome, start and end positions, mean eigenvector value, segment ID, sign, and compartment label. The second was a sorted list of all unique segment boundaries, which represented the genomic positions of compartment transitions.

To separate the data into A- and B-compartments, a simple filtering step was uses. For each genomic interval, the filter compared the compartment label (A or B) with the sign of its position relative to the nearest transition. Intervals that matched the expected direction were collected into one set, while those that fell on the opposite side were collected into another. In practice, this was done with the following code:
``` python 
    A_val = result[
...
```
This logic ensured that the data was consistently separated into values supporting the A-compartment and values supporting the B-compartment, based on both the compartment label and the position relative to the transition point.

To verify that our compartment analysis was not biased by compartment assignment, we performed a negative control experiment. Normally, the data are separated into A- or B-compartments (as described in the methods section). In the control analysis, however, the data were instead partitioned according to compartment transitions — whether regions lay on an A to B or B to A boundary. Because Hi-C data reflect DNA contacts independently of strand orientation, no systematic difference was expected between the two directions.

As an additional safeguard, the same experiment was repeated for CpG islands, repeats and GC content. Filtering was carried out with the following code snippet, which reverses the logic of the standard A/B assignment:
...
To evaluate the availability of data across genomic distances and mitigate potential bias in downstream analyses, the reverse cumulative counts of compartment edge half-distances was calculated . First, edges were identified at compartment transitions between A and B (in either direction). For each consecutive pair of such edges, we measured the genomic distance between their start positions and divided it by two, yielding the half-distance between edges.

The distances has then half and computed the reverse cumulative count for each bin, which represented the number of edges with half-distances greater than or equal to that bin. This approach effectively measures the number of edges capable of contributing data at a given spatial scale. By using this metric, it allowed us to normalize other measurements by the available data density and reducing the risk that apparent trends were driven by varying numbers of contributing edges at different distances rather than true biological effects. Counts were calculated up to 2 Mb to encompass the maximum observed separation.
...
## Aligning GC content to edges in Macaca

The result of compared GC content between A- and B- compartments for chromosome 8 and chromosome X across five Macaca cell types: fibroblasts, sperm, round spermatids, pachytene spermatocytes, and spermatogonia. Chromatin compartments were defined from A to B and B to A structural transitions, and GC content was measured separately for each compartment.
...
**Stage-specific enrichment of CpG island–associated chromatin transitions across A/B compartments in *Macaca*.**  
The figure displays chromatin transitions align to CpG islands across developmental stages of spermatogenesis. Plots are arranged in two columns: chromosome 8 (left) and chromosome X (right). Rows correspond to cell types, from top to bottom: fibroblasts, spermatogonia, pachytene spermatocytes, round spermatids, and mature sperm. On chromosome 8, compartment A transitions are shown in green and compartment B in red. On chromosome X, compartment A is represented in purple and compartment B in blue. The figure highlights gradual, stage-dependent variation in the compartmental distribution of CpG-associated transitions, with a tendency toward increased A-compartment representation during the later stages of spermatogenesis—particularly on chromosome 8..
...
chromosome 8 can be seen on graphs to the let, and chromosome x can be seen on graphs to the right. Rows represent different cell types: fibroblasts, spermatogonia, pachytene spermatocytes, round spermatids, and mature sperm. In each panel, nested repeat counts were mapped relative to transition sites, and values in each distance bin were normalized by the number of transition sites that could contribute to that bin.  

:::

## Control to with A- andf B-compartment
The outcomes of the control analysis for simple and nested repeats are presented in Figures @fig-simple_repeat_contol and @suppfig-nested_repeat_contol. Across all tissues, the distributions for both chromosome 8 and chromosome X show identical counts within each bin.  

This consistency indicates that the choice of chromosome split does not introduce a biological difference in the observed patterns. In other words, the apparent symmetry between the A to B and B to A transitions confirms that the results are not biased by the direction of the split, validating the robustness of the control setup.  


::: {#fig-simple_repeat_contol}
![](illustrations/simple_repeat/simple_repeat_chr8_vs_chrx_normalized_poisson_CI_allaxes_control.png){width=100%}

 Control analysis of simple repeats. The distributions for chromosome 8 and chromosome X show identical counts across bins in all tissues, indicating no biological difference introduced by this chromosome split.
:::

When running the control analysis with GC content, we plotted all five tissues for both chromosome 8 and chromosome X (Figure @fig-GC_content_contol).  

For chromosome 8, no significant differences were observed between the A to B and B to A splits across tissues, with the exception of spermatogonia, where a deviation appeared in the region spanning 0.25 Mb to 1.25 Mb.  

For chromosome X, the results varied by tissue. In fibroblasts, the B to A split showed consistently higher GC content compared to A to B within the 0–1 Mb region. A similar pattern was observed in sperm, where GC content remained higher in B to A. By contrast, in round spermatids and pachytene spermatocytes, there were no significant differences between the two splits. In spermatogonia, however, GC content was again higher in B to A compared to A to B.  


::: {#fig-GC_content_contol }
![](illustrations/gc_compartment_analysis_chrX_chr8_combined_control_20250925_091502.png){width=100%}

 Control analysis of GC content for chromosomes 8 and X across five tissues. For chromosome 8, no significant differences between A to B and B to A were detected except in spermatogonia (0.25–1.25 Mb). For chromosome X, higher GC content was observed in B to A compared to A to B in fibroblasts (0–1 Mb), sperm (0–1 Mb), and spermatogonia, while round spermatids and pachytene spermatocytes showed no differences.
:::

The resulting profiles in Figure @fig-CPG_normalized_chr_ab_new illustrate the distribution of CpG islands relative to compartment transitions (A to B vs. B to A) on chromosomes 8 and X across multiple cell types.

On chromosome 8, fibroblasts and sperm show a pronounced asymmetry: between ~0.5 and 1.5 Mb from transition edges, CpG islands are more abundant in regions undergoing A to B transitions than in those switching from B to A. By contrast, in spermatogonia and pachytene spermatocytes, CpG islands display a more balanced distribution, with comparable counts in both transition directions across the full range of distances examined.

On chromosome X, no strong directional bias is detected. Across all tissues analyzed—including fibroblasts, spermatogonia, pachytene spermatocytes, round spermatids, and sperm—the counts of CpG islands at A to B and B to A transitions remain similar across the 0–2 Mb distance range. This uniformity suggests that, unlike chromosome 8, the X chromosome does not exhibit a consistent asymmetry in the distribution of CpG islands relative to compartment switching.

Together, these results indicate that CpG islands are not randomly associated with chromatin transitions but instead show chromosome- and stage-specific patterns. Chromosome 8 exhibits directional enrichment of CpG islands, particularly in fibroblasts, sperm, and round spermatids, while the X chromosome appears more symmetric, lacking a consistent bias between A to B and B to A transitions.


::: {#fig-CPG_normalized_chr_ab_new}
 ![](illustrations/cpgislands/cpgislands_chr8_vs_chrx_normalized_poisson_CI_allaxes_negative_controal_AB_ba.png){width="100%"} 

Negative control analysis of CpG islands on chromosome 8 across five macaque cell types (fibroblast, spermatogonia, pachytene spermatocyte, round spermatid, and sperm). A to B (AB) and B to A (BA) compartment transitions were compared.




**Negative control analysis of CpG island proximity to A/B compartment transitions on chromosome 8 across spermatogenic stages in *Macaca*.**  
This control analysis compares CpG island frequencies near directional compartment transitions—specifically transitions from A to B (AB) and from B to A (BA) rather than focusing on static A or B compartments, as in the main analysis. For each of the five cell types (fibroblast, spermatogonia, pachytene spermatocyte, round spermatid, and mature sperm), genomic regions undergoing compartment status changes were identified, and CpG island counts were measured relative to these transition edges.

Plots follow the same structure as in the main figure: chromosome 8 (left column) and chromosome X (right column), with rows arranged from fibroblast (top) to sperm (bottom). On chromosome 8, Ab to B transitions are shown in green and B to A in red; on chromosome X, A-to-B is shown in purple and B-to-A in blue.

In fibroblasts on chromosome 8, higher CpG island counts were observed in the A to B category between ~0.8 and 1.4 Mb. Similar differences between AB and BA transitions were also present in pachytene spermatocytes, round spermatids, and sperm, particularly from 0 to ~1.2 Mb. On chromosome X, CpG island counts for AB and BA transitions were comparable across all stages.
...
As earlier established the GC content is a strong methode for identifying the A/B compertments, with A-compartments being GC rich and B-compartmets being GC poor. The analysis done in this paper of the macaque chromosome 8 shows the same realtionship in all five cell types examined, as seen on @fig-GC.  In contrast, chromosome X departs from this pattern in several ways. First, the precetags diffrence of GC content between A and B is reduced in spermatogonia, pachytene spermatocytes, round spermatids, and sperm. Second, fibroblasts show an inversion higher GC in B-compartment than in A-compartment. Despite this relationship seen in chromosome 8, it is important to have in mind, that this approach has three main limitations: (i) edge positions are localized only to 100 kb, so features within ~±50 kb of a edges may be blurred by binning; (ii) the amount of data points declines with increasing distance due to many of the edges close proximyty to each other, which widens confidence intervals at longer distances even after normalization; and lastly (iii) because PC1 was phased using GC to orient A/B compartments. 

The chromosome 8 follows the classic GC A/B relationship while chromosome X does not highlights an important distinction: GC content is a robust predictor of compartment state on autosomes but not necessarily on sex chromosomes. The attenuated contrast and apparent fibroblast reversal suggest that on chromosome X, base composition is a strong prior for compartmentalization but not a determinant under all conditions. Regulatory programs such as dosage compensation, sex-specific chromatin environments, and the unusual sequence architecture of the X chromosome can override the typical GC to A mapping, compressing or inverting the signal.
...
((
This suggests that CpG islands are preferentially associated with loci moving from active to repressive chromatin environments in these contexts. In round spermatids, a similar enrichment is observed closer to the transition site (0–1 Mb), again with higher counts in A to B compared to B to A))





## Control analysis of A to B vs. B to A transitions

The negative control analysis was designed to test whether directional compartment assignments (A to B versus B to A) could introduce spurious structure into our results. Since Hi-C captures contact frequencies independently of strand orientation, the null expectation is symmetry: features such as repeats, GC content, or CpG islands should be distributed equivalently across both types of transitions. In large part, our findings support this expectation. For example, simple and nested repeats showed no systematic differences across transition categories or tissues, confirming that our splitting strategy did not artificially bias these features. Similarly, on the X chromosome, CpG islands and GC content exhibited balanced distributions across A to B and B to A transitions in most tissues.

However, several deviations from symmetry were observed. Most notably, chromosome 8 displayed stage-specific enrichment of CpG islands in A to B transitions, particularly in fibroblasts, sperm, and round spermatids. Likewise, GC content differences emerged on chromosome X in some tissues, with B to A regions showing consistently elevated values. These patterns raise an important interpretive challenge: are they biologically meaningful, or could they reflect artifacts introduced by the partitioning scheme itself?

One possible explanation is that the asymmetries arise from the structure of the data splitting procedure. By conditioning on compartment boundaries, the analysis inherently emphasizes regions undergoing dynamic changes in chromatin state. If features such as CpG islands or GC-rich domains are unevenly distributed relative to those boundaries, then separating regions into A to B versus B to A could exaggerate apparent differences even if no true directional bias exists. In other words, what appears to be a biological signal may instead reflect the geometry of the compartment transition and how bins are aligned across the genome.

At the same time, the tissue- and chromosome-specific nature of these asymmetries argues against a purely technical artifact. If the differences were driven solely by the method of splitting, one would expect them to occur broadly and consistently across tissues and chromosomes. Instead, the enrichment of CpG islands in A to B regions is largely confined to chromosome 8 and restricted to particular developmental stages, suggesting that at least part of the signal reflects genuine biology. This interpretation is supported by the absence of comparable asymmetries on the X chromosome, which serves as an internal control.

Taken together, these results highlight both the strength and the limitation of the control design. On the one hand, the null expectation of symmetry holds for many features, providing reassurance that our compartmental analyses are not systematically biased. On the other hand, the exceptions underscore that care must be taken when interpreting direction-specific enrichments. Some patterns, particularly those restricted to individual chromosomes and developmental stages, may represent true biological processes—for instance, preferential positioning of CpG islands at A to B transitions during chromatin remodeling. Others, however, may be artifacts of the binning and splitting procedure. Distinguishing between these possibilities will require further analyses, such as permuted controls, higher-resolution maps, or independent validation datasets.

## Reproduction Attempt of GPN
...
Despite these efforts, the model continued to encounter out-of-memory errors during execution. Even with eight GPUs, the memory demands of the GPN model remained prohibitive under the given cluster configuration. These persistent failures highlighted a significant limitation: the model's architecture and resource demands exceeded the practical limits of the available hardware.



Our attempts to reproduce the Genome-Pretrained Network (GPN) workflow highlighted both the promise of the approach and the challenges of applying it under limited computational resources. While we successfully generated pretraining-compatible datasets and partially executed the workflow, full-scale training was hindered by two main factors: insufficient documentation and substantial hardware requirements.  

The original GPN study relied on multiple high-memory GPUs (A100s with 80–100 GB RAM), whereas our available infrastructure consisted of GPUs with less than half this capacity. This mismatch resulted in repeated out-of-memory errors and prevented us from scaling the model to the level reported in the original publication. Although we explored workarounds such as chromosome-level partitioning, these only enabled partial execution and did not allow for comprehensive evaluation.  

This outcome underscores a broader limitation of cutting-edge genomics deep learning models: their reliance on specialized hardware makes reproducibility difficult for smaller research groups. At the same time, the successful preparation of compatible datasets and the partial execution steps demonstrate that the workflow is adaptable in principle. Future work should prioritize clearer documentation and optimization strategies that reduce the model’s hardware footprint, thereby lowering barriers to entry for the wider community.  


# Conclussion 

Analyses of GC content, CpG islands, and simple repeats relative to A/B compartments and their transitions reveal both stable principles of 3D genome organization and notable exceptions. On autosomes such as chromosome 8, the canonical link between GC-rich, CpG island–dense regions and active A-compartments is maintained across developmental stages, underscoring the reliability of the compartment-calling strategy. In contrast, the macaque X chromosome departs from this pattern, showing attenuated or inverted correlations that reflect its distinctive regulatory environment and evolutionary history.

CpG islands show dynamic, stage-specific associations with compartment transitions, becoming enriched in A to B regions during later stages of spermatogenesis when chromatin is extensively reorganized. Simple repeats, by comparison, display reciprocal biases between autosomes and the X chromosome and accumulate at compartment boundaries, consistent with potential structural or modulatory roles.

Control analyses provide important context for these observations. Symmetry between A to B and B to A transitions holds in many cases, yet selective deviations—such as CpG island enrichment on chromosome 8 and GC asymmetries on the X—raise the possibility that both true biological processes and methodological artifacts are contributing to the signal. Independent validation with orthogonal datasets and higher-resolution maps will be crucial for resolving this ambiguity.

Altogether, the findings indicate that while GC content and CpG islands are strong predictors of compartment identity on autosomes, their predictive power is context-dependent and less reliable on the X chromosome. Sequence composition, repeats, and chromatin structure emerge as deeply interconnected, shaped by both general organizing principles and chromosome-specific constraints. Future work integrating chromatin marks, transcriptional activity, and experimental perturbations will be essential to separate methodological effects from genuine biology and to refine models of how sequence features guide higher-order genome folding.

## futher investigation 
This study establishes a foundation for linking sequence features with chromatin compartments during spermatogenesis, but several directions remain open. A key next step is to integrate protein-binding data, particularly CTCF, to test whether compartment edges coincide with architectural anchor points and how these relationships change across developmental stages.

Recombination rates also warrant investigation. Because meiotic hotspots are shaped by chromatin environment, assessing crossover frequency near compartment boundaries could connect 3D genome structure with germline evolution. Structural variants such as inversions provide another opportunity, as they can reposition or disrupt boundaries and reveal causal effects of sequence context.

Finally, higher resolution will be essential. Our current 100-kb compartment calls provide a broad view but obscure finer features. Moving to 10-kb resolution would sharpen boundary localization and enable more precise comparisons with sequence elements, regulatory sites, and recombination maps. Ongoing efforts in this direction within the group will provide an important resource for these future analyses.
